...
class GNNModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        scheduler = {'scheduler': self.lr_scheduler,
                     'monitor': 'val_loss',
                     'interval': 'epoch',
                     'frequency': 1}
...
if __name__ == '__main__':
    ...
    # Inputs: bs: 40, do: 0.25, depth: 4, lr: 0.0001, num_epochs: 1500
    parser = argparse.ArgumentParser()
    ...
    # Paths and folders
    path             = 'yourPath'
    folder           = path + 'output_3D/Patches'
    featureFolder    = os.path.join(folder, 'yourFeaturefolder')
    the_time         = str(time.time())
    output           = path + f'output_3D/GNN_encodings'
    model_name       = f'{layer_type}_test_cv'+str(c) + '_run'+str(r)
